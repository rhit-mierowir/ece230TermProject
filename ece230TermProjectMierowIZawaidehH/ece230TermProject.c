...
//Water Motor Constants
#define WATER_PUMP_PORT P2
#define WATER_PUMP_MASK (0b1 << 7)

_Bool watering = 0; //is the watering system currently on
...
void configPump(){
    //Set output, DIR=1
    WATER_PUMP_PORT->DIR |= WATER_PUMP_MASK;

    //Set to default GPIO Pins SEL=0,0
    WATER_PUMP_PORT->SEL0 &= ~WATER_PUMP_MASK;
    WATER_PUMP_PORT->SEL1 &= ~WATER_PUMP_MASK;

    //Start with lights off OUT=0
    WATER_PUMP_PORT->OUT &= ~WATER_PUMP_MASK;
}
...
//Handling Delays internally and not with timers.
